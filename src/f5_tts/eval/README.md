...
`pick_demo_samples.py` ranks the SAP / L2-ARCTIC pairs generated by both RTFree-F5 and the F5-TTS oracle-transcript
baseline (largest WER gap in favour of RTFree-F5, identity and naturalness preserved, 2.5-8 s) and writes the top-k
triples (ground-truth target, F5-TTS, RTFree-F5, plus the reference) loudness-matched at 24 kHz:

```bash
python src/f5_tts/eval/pick_demo_samples.py -t sap_dev -d <SAP_ROOT> \
    --rtfree_dir <GEN_WAV_DIR of RTFree_F5, sap_dev_rtfree> --oracle_dir <GEN_WAV_DIR of F5TTS_v1_Base, sap_dev_oracle> \
    -o demo/sap -k 5
```
